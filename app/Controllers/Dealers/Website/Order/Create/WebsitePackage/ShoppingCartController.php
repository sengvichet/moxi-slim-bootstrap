 ...
 * @link     http://lamp-dev.com
 */
class ShoppingCartController extends CreateOrderController
{
    /**
    ...
    {        
        ...
        $data['action'] = [
            'form' => $this->getService('config')->get()['routes']['order-shopping-cart'],
        ];
        $data['percentage'] = (empty($data['order']))
            ? $this->getPercentage([]) : $this->getPercentage($data['order']);

        return $this->render('Dealers/website/order/website-package/shopping-cart.twig', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$request->isPost()) {
            return $this->goBack('shopping-cart');
        }

        $data = $request->getParams();
        $data['user_id'] = $this->getService('session')->user['id'];
        $status = $this->storeCosts($data, 'ecommerce');
        if (!$status) {
            return $this->goBack('shopping-cart');
        }
        if ($data['ecommerce'] == 1) { // 'Yes'
            $data['pricing'] = 1; // 'Yes'
            $status = $this->storeCosts($data, 'pricing');
        }

        return $this->handleNextAction(
            $data['action'],
            'spec-sheets',
            ($data['ecommerce'] == 2) ? 'pricing' : 'end'
        );
    }
}
