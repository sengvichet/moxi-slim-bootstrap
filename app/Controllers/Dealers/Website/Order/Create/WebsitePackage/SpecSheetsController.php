 ...
 * @link     http://lamp-dev.com
 */
class SpecSheetsController extends CreateOrderController
{
    /**
    ...
    {        
        ...
        $data['action'] = [
            'form' => $this->getService('config')->get()['routes']['order-spec-sheets'],
        ];
        $data['percentage'] = (empty($data['order']))
            ? $this->getPercentage([]) : $this->getPercentage($data['order']);

        return $this->render('Dealers/website/order/website-package/spec-sheets.twig', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$request->isPost()) {
            return $this->goBack('spec-sheets');
        }

        $data = $request->getParams();
        $data['user_id'] = $this->getService('session')->user['id'];
        $status = $this->storeCosts($data, 'specsheets');
        if (!$status) {
            return $this->goBack('spec-sheets');
        }

        return $this->handleNextAction($data['action'], 'number-of-products', 'shopping-cart');
    }
}
