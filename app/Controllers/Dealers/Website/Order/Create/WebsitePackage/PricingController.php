 ...
 * @link     http://lamp-dev.com
 */
class PricingController extends CreateOrderController
{
    /**
    ...
    {        
        ...
        $data['action'] = [
            'form' => $this->getService('config')->get()['routes']['order-pricing'],
        ];
        $data['percentage'] = (empty($data['order']))
            ? $this->getPercentage([]) : $this->getPercentage($data['order']);

        return $this->render('Dealers/website/order/website-package/pricing.twig', $data);
    }
    ...
    public function store()
    {
        ...
        return $this->handleNextAction($data['action'], 'shopping-cart', 'end');
    }
}
