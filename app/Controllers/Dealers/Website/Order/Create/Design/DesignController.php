 ...
 * @link     http://lamp-dev.com
 */
class DesignController extends CreateOrderController
{
    /**
    ...
    public function index()
    {
        ...
        $data['action'] = [
            'form' => $this->getService('config')->get()['routes']['order-design'],
        ];
        $data['percentage'] = (empty($data['order']))
            ? $this->getPercentage([]) : $this->getPercentage($data['order']);

        return $this->render('Dealers/website/order/design/design.twig', $data);
    }
    ...
    public function store()
    {
        ...
        return $this->handleNextAction($data['action'], 'business-information', 'website-information');
    }
}
